export interface TranscriptWord { text: string; start: number; end: number; confidence?: number; language?: string; }
// TranscriptWord imported locally above
...
// ── Honcut extensions (from REST API) ──

export interface CaptionCue {
  id: string; project_id: string; start_frame: number;
  duration_frames: number; text: string; style: string; created_at: string;
}
export interface CaptionStyle {
  fontFamily: string; fontSize: number; fontWeight: number;
  color: string; backgroundColor: string; outlineColor: string;
  outlineWidth: number; position: "top"|"center"|"bottom";
  alignment: "left"|"center"|"right"; padding: number;
  lineHeight: number; opacity: number;
}
export const DEFAULT_CAPTION_STYLE: CaptionStyle = {
  fontFamily: "'Noto Sans SC', sans-serif", fontSize: 42, fontWeight: 700,
  color: "#fff", backgroundColor: "rgba(0,0,0,0.55)", outlineColor: "#000",
  outlineWidth: 2, position: "bottom", alignment: "center",
  padding: 10, lineHeight: 1.4, opacity: 1,
};
export function parseCaptionStyle(json: string): CaptionStyle {
  try { return { ...DEFAULT_CAPTION_STYLE, ...JSON.parse(json) }; }
  catch { return { ...DEFAULT_CAPTION_STYLE }; }
}
export function serializeCaptionStyle(style: CaptionStyle): string {
